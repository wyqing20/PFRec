...
def format_metric(result_dict: Dict[str, Any]) -> str:
    ...
    if 'AUC' in result_dict:
        format_str.append('{}:{:<.4f}'.format('AUC', result_dict['AUC']))
    if 'DIS_AUC' in result_dict:
        format_str.append('{}:{:<.4f}'.format('DIS_AUC', result_dict['DIS_AUC']))
    if 'acc' in result_dict:
        format_str.append('{}:{:<.4f}'.format('ACC', result_dict['acc']))
    if 'f1_macro' in result_dict:
        format_str.append('{}:{:<.4f}'.format('F1_Macro', result_dict['f1_macro']))
        
    metrics = np.unique([k.split('@')[0] for k in result_dict.keys() if 'NDCG'  in k or 'HR' in k])
    topks = np.unique([int(k.split('@')[1]) for k in result_dict.keys()  if 'NDCG'  in k or 'HR' in k])
